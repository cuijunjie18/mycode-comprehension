...
constexpr int N = 1e5 + 5, M = 30, inf = 0x7fffffff;

int n, m, cnt, ans = inf;
int lg[M],head[N],f[N],dep[N],fa[N][M],maxn[N][M],maxx[N][M];
long long minn;

struct Edge { int from, to, val, flag; } a[N << 2];

struct Node { int next, to, val; } e[600005];
...
void init()
{
    cin >> n >> m;
    lg[0]=1;
    for(int i = 1; i <= 20; ++i) lg[i] = lg[i - 1] * 2;
    for(int i = 1; i <= n; ++i) f[i] = i, head[i] = -1;
...
void kruskal()
{
    for(int i = 1, cntt = 0; i <= m; ++i)
    {
        ...
        f[v] = u,
        a[i].flag = 1,
        add(a[i].from, a[i].to, a[i].val),
        add(a[i].to, a[i].from, a[i].val);
        if(++cntt == n - 1) break;
    }
}
...
void dfs(int u,int father,int depth)
{
    ...
    for(int i=1; ; ++i)
    {
        ...
        if(maxn[u][i - 1] == maxn[fa[u][i - 1]][i - 1])
        {
            maxx[u][i]=max(maxx[u][i-1],maxx[fa[u][i-1]][i-1]);
        }
        else
        {
            maxx[u][i] = max(min(maxn[u][i - 1], maxn[fa[u][i - 1]][i - 1]),
            max(maxx[u][i - 1], maxx[fa[u][i - 1]][i - 1]));
        }
    }
...
int lca(int u,int v)
{
    ...
    int k = dep[u] - dep[v];
    for(int i = 0; i <= 17; ++i) if(k >> i & 1) u = fa[u][i];
    if(u == v) return u;
...
{
    int m1 = 0, m2 = 0, k = dep[u] - dep[v];
    for(int i = 0; i <= 17; ++i)
    {
        if(k >> i & 1)
        {
            m2 = max(m2, maxx[u][i]);
            if(maxn[u][i] > m1) m2 = max(m2, m1), m1=maxn[u][i];
        }
    }
    if(m1 == val) ans = min(ans, val - m2);
    else ans = min(ans, val - m1);
}
...
int main()
{
    ...
    for(int i = 1; i <= m; ++i)
    {
        ...
        int u = a[i].from, v = a[i].to, l = lca(u, v);
        solve(u, l, a[i].val),
        solve(v, l, a[i].val);
    }
    cout << minn + ans << endl;
...
#include <vector>
using namespace std;
int n, m, head[100010], cnt, dis[100010], num_dis[100010];
bool flag[100010];
struct Edge {int next, to; } a[500010];
void add (int u, int v) { a[++cnt].to = v, a[cnt].next = head[u], head[u] = cnt;}
void Dijkstra ()
{
...
void init ()
{
    scanf("%d%d", &n, &m), num_dis[1] = 1;
    for (int i = 2; i <= n; ++i) dis[i] = 0x7fffffff;
    for (int i = 1, u, v, w; i <= m; ++i) scanf("%d%d", &u, &v), add(u, v), add(v, u);
}
int main ()
{
    init(), Dijkstra();
...
int n, m, s, t, fa[N], ans = 1;
double e, dis[N];
bool flag[N], vis[N], ontree[N];
struct Edge
{
    int cnt, head[N], next[N], to[N];
    double val[N];
...
struct Tree
{
    ...
    int merge (int x, int y)
    {
        ...
        if (a[x] < a[y]) swap(x, y);
        int p = ++cnt;
        lc[p] = lc[x], a[p] = a[x], rc[p] = merge(rc[x], y);
...
void dfs1(int x)
{
    ...
    {
        if (vis[e2.to[i]]) continue;
        if (fabs(dis[e2.to[i]] - dis[x] - e2.val[i]) < eps)
        {
            fa[e2.to[i]] = x,
            ontree[i] = 1,
            dfs1(e2.to[i]);
...
{
    vis[x] = 1;
    if (fa[x]) tree.rt[x] = tree.merge(tree.rt[x], tree.rt[fa[x]]);
    for (int i = e2.head[x]; i; i = e2.next[i])
    {
        if (fa[e2.to[i]] == x && !vis[e2.to[i]])
        {
            dfs2(e2.to[i]);
        }
    }
...
int main ()
{
    ...
    {
        Node a = q.top(); q.pop();
        if (flag[a.id]) continue;
        flag[a.id] = 1, dis[a.id] = a.val;
        for(int i = e2.head[a.id]; i; i = e2.next[i])
        {
            q.push({e2.to[i], a.val + e2.val[i]});
        }
    }
    if (e - dis[s] < 0) return printf("0\n"), 0;
    e -= dis[s], dfs1(t);
    for (int i = 1; i <= n; ++i)
    {
        if (flag[i])
        {
            for (int j = e1.head[i]; j; j = e1.next[j])
            {
                if (!ontree[j] && flag[e1.to[j]])
                {
                    tree.rt[i] = tree.merge(tree.rt[i], tree.ins({e1.to[j], dis[e1.to[j]] + e1.val[j] - dis[i]}));
                }
            }
        }
    }
    ...
    dfs2(t);
    if (tree.rt[s]) q.push({tree.rt[s], dis[s] + tree.a[tree.rt[s]].val});
    while (!q.empty())
    {
        Node a = q.top();
        q.pop();
        ...
        if (tree.lc[a.id])
        {
            q.push({tree.lc[a.id], a.val - tree.a[a.id].val + tree.a[tree.lc[a.id]].val});
        }
        if (tree.rc[a.id])
        {
            q.push({tree.rc[a.id], a.val - tree.a[a.id].val + tree.a[tree.rc[a.id]].val});
        }
        int x = tree.rt[tree.a[a.id].id];
        if (x) q.push({x, a.val + tree.a[x].val});
    }
    printf("%d\n", ans);
    return 0;
}
...
在跑bfs时为图打上层数标记，在dfs时实现多路增广（可运用当前弧优化进一步优化）；
...
int n, m, s, t, cnt = 1, head[210], dep[210];
ll ans, sum;
struct Edge {int next, to; ll val; } a[10010];
...
ll dinic (int u, ll sum)
{
    if (u == t) return sum;
    ll j = sum, k;
    for(int i = head[u], v; i && j ; i = a[i].next)
    {
        v = a[i].to;
        if (a[i].val && dep[v] == dep[u] + 1)
        {
            k = dinic(v, min(a[i].val, j));
            if (!k) dep[v] = 0;
            a[i].val -= k, a[i ^ 1].val += k, j -= k;
        }
    }
    return sum - j;
}
...
{
    init();
    while (bfs()) while((sum = dinic(s, inf))) ans += sum;
    cout << ans << endl;
    return 0;
}
```
...
int n, m, s, t, cnt = 1, head[210], dep[210], gap[210];
ll ans, sum;
struct Edge {int next, to; ll val; } a[10010];
void add (int u, int v, ll w)
{
...
void bfs()
{
    ...
    while(q.size())
    {
        ...
        {
            v = a[i].to;
            if (dep[v] == -1) dep[v] = dep[u] + 1, ++gap[dep[v]], q.push(v);
        }
    }
}
...
ll dfs(int u, ll sum)
{
    ...
    {
        v = a[i].to;
        if (a[i].val && dep[v] + 1 == dep[u])
        {
            k = dfs(v, min(a[i].val, sum - j));
            if (k) a[i].val -= k, a[i ^ 1].val += k, j += k;
            if (j == sum) return j;
        }
    }
...
```cpp
#include <cstdio>
int n, m, e, cnt, head[30010], ans, f[30010], s[30010];
struct Node {int to, next; } a[30010];
...
{
    scanf("%d%d%d", &n, &m, &e);
    for(int i = 1, u, v; i <= e; ++i) scanf("%d%d", &u, &v), add(u, v);
    for(int i = 1; i <= n; ++i) if(KM(i, i)) ++ans;
    printf("%d\n", ans);
    return 0;
}
```
...
int n, m, e, s, t, cnt = 1, head[10005], dep[10005];
ll ans, sum;
struct Edge {int next, to; ll val; } a[1000005];
void add (int u, int v, ll w)
{
    a[++cnt].to = v,
    a[cnt].val = w,
...
int head[5005], vis[5005], dis[5005], f[5005], fa[5005];

struct Edge { int next, to, flow, cost; } a[100010];
...
bool spfa()
{
    ...
    queue <int> q;
    q.push(s), dis[s] = 0, vis[s] = 1, f[s]=0x7fffffff;
    while (q.size())
    {
        int u = q.front();
        q.pop(), vis[u] = 0;
